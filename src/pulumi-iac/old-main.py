...
igw = aws.ec2.InternetGateway(
    "poridhi-exam-igw", vpc_id=vpc.id, tags={"Name": "poridhi-exam-igw"}
)

# route table with igw route
rtb = aws.ec2.RouteTable(
    "poridhi-exam-rtb",  # no need to match this name and tag name
    vpc_id=vpc.id,
    routes=[{"cidr_block": "0.0.0.0/0", "gateway_id": igw.id}],  # gateway attached
    tags={
        "Name": "poridhi-exam-rtb"
    },  # this tag name is populated as the resource name in aws
)

# rtb subnet association
rtb_sb_association1 = aws.ec2.RouteTableAssociation(
    "poridhi-rta-1a", subnet_id=subnet1.id, route_table_id=rtb.id
)

rtb_sb_association2 = aws.ec2.RouteTableAssociation(
    "porisdi-rta-1b", subnet_id=subnet2.id, route_table_id=rtb.id
)
...
ec2_instance1 = aws.ec2.Instance(
    "poridhi-exam-instance-backend-1a",
...
ec2_instance2 = aws.ec2.Instance(
    "poridhi-exam-instance-backend-1b",
    instance_type="t2.micro",
    ami=ami_id,
...
ec2_instance2 = aws.ec2.Instance(
    "poridhi-exam-instance-client-1a",
    instance_type="t2.micro",
    ami=ami_id,
    subnet_id=subnet2.id,
    vpc_security_group_ids=[security_group.id],
...
target_group_tg = aws.lb.TargetGroup(
    "poridhi-exam-tg",
    vpc_id=vpc.id,
    protocol="HTTP",
    port=80,
...
tg_attachment1 = aws.lb.TargetGroupAttachment(
    "poridhi-exam-tg-attachment-1a",
    target_group_arn=target_group_tg.arn,
    target_id=ec2_instance1.id,
    port=80,
)

tg_attachment2 = aws.lb.TargetGroupAttachment(
    "poridhi-exam-tg-attachment-1b",
    target_group_arn=target_group_tg.arn,
